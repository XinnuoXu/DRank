...
class DialogueAgent(object):
    ...
    def start_call(self, session_id, domainSimulatedUsers=None, maxNumTurnsScaling=1.0, start_domain=None):
        ...
        print ("PYDAIL_LOG\t>> NEW DIALOGUE SESSION. Number: "+str(self.NUM_DIALOGS))
    ...
    def continue_call(self, asr_info, uact=None, domainString=None, domainSimulatedUsers=None):
        ...
        #print (state.printUserActs(currentDomain))
...
#         assert(not self.ENDING_DIALOG)
    ...
    def _agents_semo(self, act, uact_domain=None):
        '''
        ...
        # @XINNUO: When the input is user_act
        if self.semo_manager is not None:
            if uact_domain != None:
                logger.dial("Domain of US: "+uact_domain)
                act.act = "u_" + act.act
                for item in act.items:
                    if item.slot == None and item.val == u'dontcare':
                        item.slot = u'dontcare'
                prompt_str = self.semo_manager.generate(act, uact_domain)
                print ("PYDAIL_LOG\tUSER: ", prompt_str)
            else:
                logger.dial("Domain with CONTROL: "+self.topic_tracker.operatingDomain)
                prompt_str = self.semo_manager.generate(act, domainTag=self.topic_tracker.operatingDomain)
                print ("PYDAIL_LOG\tSYSTEM: ", prompt_str)
        else:
            prompt_str = None 
        return prompt_str                      
    ...
    def check_if_user_bye(self,obs):
        ...
        for ob in obs:
            ...
            # simulated user input
                ...
                sentence,_ = ob, None    
            rBYE = "(\b|^|\ )(thank|okay|ok|bye|goodbye|that'*s*\ (is\ )*all)(\s|$|\ |\.)"
