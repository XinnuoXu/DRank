...
if __name__ == '__main__':
	...
	if sys.argv[1] == "data":
		...
		for filename in os.listdir("./generator/"):
			if filename.startswith("dialogue-model.epoch"):
				epoch = int(filename.split(".")[1].replace("epoch", ""))
				gen_len = int(filename.split(".")[2].replace("gen", ""))
				if gen_len not in final_epoch:
					final_epoch[gen_len] = epoch
				else:
					if epoch > final_epoch[gen_len]:
						final_epoch[gen_len] = epoch
		for item in final_epoch:
			filename = "dialogue-model.epoch" + str(final_epoch[item]) + ".gen" + str(item)
	...
	if sys.argv[1] == "train":
		for turn_num in range(1, 10):
			...
			for item in neg_examples_dict:
				#neg_examples += random.sample(neg_examples_dict[item], 300)
