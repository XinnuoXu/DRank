...
def check_existing_pt_files(opt):
    ...
    # or mixing them up.
    if opt.pre_train:
        check_list = ['train', 'valid', 'vocab']
    else:
        check_list = ['train', 'valid']
    for t in check_list:
...
def build_save_in_shards(src_corpus, tgt_corpus, fields,
    ...
    src_iter = inputters.ShardedTextCorpusIterator(
        ...
        "src", opt.max_shard_size, corpus_type, opt.pre_train)
    tgt_iter = inputters.ShardedTextCorpusIterator(
        tgt_corpus, opt.tgt_seq_length_trunc,
        "tgt", opt.max_shard_size, corpus_type, opt.pre_train, assoc_iter=src_iter)
