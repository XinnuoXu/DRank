...
class LossComputeBase(nn.Module):
    ...
    def monolithic_compute_loss(self, batch, output, attns, rewards):
        """
        ...
        _, batch_stats = self._compute_loss(batch, rewards, **shard_state)
    ...
    def sharded_compute_loss(self, batch, output, attns,
        ...
        for shard in shards(shard_state, shard_size):
            loss, stats = self._compute_loss(batch, rewards, **shard)
...
class NMTLossCompute(LossComputeBase):
    ...
    def _compute_loss(self, batch, rewards, output, target):
        ...
        rewards = torch.cat([rewards for i in range(0, target.size()[0])], 0)
        rewards = torch.cat([rewards for i in range(0, scores.size()[1])], 0).reshape(scores.size()[1], -1)
        rewards = torch.t(rewards)

        entropy = torch.distributions.categorical.Categorical(scores).entropy()
        entropy = torch.cat([entropy for i in range(0, scores.size()[1])], 0).reshape(scores.size()[1], -1)
        entropy = torch.t(entropy)

        loss = self.criterion(scores * rewards + entropy * 1.5, gtruth)
