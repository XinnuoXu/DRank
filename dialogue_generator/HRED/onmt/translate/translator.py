...
class Translator(object):
    ...
    def _translate_batch(self, batch, data):
        ...
        memory_bank_list, enc_final_list = self.model.word_level(src)
        sen_src = torch.cat(enc_final_list, 0).reshape(len(enc_final_list), src.size()[1], -1)
        enc_final, _ = self.model.sentence_encoder(sen_src, None)

        enc_states, memory_bank = self.model.encoder(src, src_lengths)
        #dec_states = self.model.decoder.init_decoder_state(
        #    src, memory_bank, enc_final)
        dec_states = self.model.decoder.init_decoder_state(memory_bank, enc_final)
    ...
    def _run_target(self, batch, data):
        ...
        _, memory_bank = self.model.encoder(src, src_lengths)
        dec_states = self.model.decoder.init_decoder_state(memory_bank, enc_final)
