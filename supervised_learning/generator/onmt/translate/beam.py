...
class Beam(object):
    ...
    def advance(self, word_probs, attn_out, beam_sample=False):
        """
        ...
        * `attn_out`- attention at the last step
        ...
        if beam_sample:
            tmp_probs = torch.exp(flat_beam_scores)
            if torch.sum(tmp_probs).item() == 0:
                best_scores, best_scores_id = flat_beam_scores.topk(self.size, 0, True, True)
            else:
                best_scores_id = torch.multinomial(tmp_probs, self.size)
                best_scores = []
                for id in best_scores_id:
                    best_scores.append(flat_beam_scores[id.item()].item())
    ...
    def random_finished(self, minimum=None):
