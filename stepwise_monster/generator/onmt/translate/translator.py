...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    kwargs = {k: getattr(opt, k)
              for k in ["beam_size", "n_best", "max_length", "min_length",
                        ...
                        "data_type", "replace_unk", "gpu", "verbose", "fast", "beam_sample"]}
...
class Translator(object):
    ...
    def _fast_translate_batch(self,
        ...
        # Encoder forward.
        src, _ = inputters.make_features(batch, 'src', data.data_type)
    ...
    def _translate_batch(self, batch, data):
        ...
        src, _ = inputters.make_features(batch, 'src', data_type)
        src_lengths = None
        if data_type == 'text':
            _, src_lengths = batch.src
        ...
        for i in range(self.max_length):
            ...
            for j, b in enumerate(beam):
                b.advance(out[:, j], beam_attn.data[:, j, :memory_lengths[j]], beam_sample=self.beam_sample)
                dec_states.beam_update(j, b.get_current_origin(), beam_size)

        # (4) Extract sentences from beam.
        ret = self._from_beam(beam, beam_sample=self.beam_sample)
    ...
    def _from_beam(self, beam, beam_sample=False):
        ret = {"predictions": [],
               "scores": [],
               "attention": []}
        for b in beam:
            n_best = self.n_best
            if beam_sample:
                scores, ks = b.random_finished(minimum=n_best)
            else:
    ...
    def _run_target(self, batch, data):
        ...
        if data_type == 'text':
            ...
            src_lengths = None
        src, _ = inputters.make_features(batch, 'src', data_type)
        tgt_in, _ = inputters.make_features(batch, 'tgt')
        tgt_in = tgt_in[:-1]
