...
class Generator(nn.Module):
    ...
    def sample(self, gen_turns, max_ctx, tag, given_turns, pre_train, model_path = "", gen_id = ""):
        ...
        while gen_turns > 0:
            ...
            # generate user_utt
            fp_test_en = open(self.path_generator + self.path_test_en + "." + gen_id, "w")
            fp_test_vi = open(self.path_generator + self.path_test_vi + "." + gen_id, "w")
            ...
            gen_uttrs = [line.strip() for line in open(self.path_generator + self.path_pred + "." + gen_id)]
...
if __name__ == '__main__':
    ...
    gen = Generator()
    contexts = gen.sample(MAX_GEN_LENGTH, MAX_CONTEXT_LENGTH, "neg", GIVEN_TURNS, False, model_path = sys.argv[1], gen_id = sys.argv[2])
