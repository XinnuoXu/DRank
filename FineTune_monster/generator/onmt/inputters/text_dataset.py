...
class TextDataset(DatasetBase):
    ...
    def get_fields(n_src_features, n_tgt_features):
        ...
        fields["rewards"] = torchtext.data.Field(
            use_vocab=False, dtype=torch.float,
            sequential=False)
...
class ShardedTextCorpusIterator(object):
    ...
    def __init__(self, corpus_path, line_truncate, side, shard_size, corpus_type, pre_train,
                 assoc_iter=None):
        """
        Args:
    ...
    def _example_dict_iter(self, line, index):
        ...
        if self.line_truncate:
            line = line[:self.line_truncate]
        rewards = 1
        if (not self.pre_train) and self.side == "src" and self.corpus_type == "train":
            rewards = float(line[0])
            line = line[1:]
        words, feats, n_feats = TextDataset.extract_text_features(line)
        example_dict = {self.side: words, "indices": index}
        if self.side == "src":
            example_dict["rewards"] = rewards
        if feats:
