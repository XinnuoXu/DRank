...
def merge_vocabs(vocabs, vocab_size=None, extra_vocab=""):
    """
    ...
    ex_vocab = load_vocabulary(extra_vocab)
    if ex_vocab:
        for v in ex_vocab:
            merged[v] = 10000
...
def make_features(batch, side, data_type='text'):
    ...
    if data_type == 'text':
        if side == 'src':
            if 'rewards' not in batch.__dict__:
                return torch.cat([level.unsqueeze(2) for level in levels], 2), None
            else:
                return torch.cat([level.unsqueeze(2) for level in levels], 2), batch.__dict__['rewards']
        else:
            return torch.cat([level.unsqueeze(2) for level in levels], 2), None
    else:
        return levels[0]
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
                ...
                tgt_vocab_path, tgt_vocab_size, tgt_words_min_frequency, extra_vocab = ""):
    """
    Args:
    ...
    if data_type == 'text':
        ...
        if share_vocab:
            ...
            merged_vocab = merge_vocabs(
                ...
                vocab_size=src_vocab_size, extra_vocab=extra_vocab)
