...
class DialogueContextDatasetReader(DatasetReader):
    ...
    def text_to_instance(self, context: str, label=None) -> Instance:  # type: ignore
        repeat = len(set(context[i] for i in range(0, len(context), 2))) / ((len(context)+1)/2.0)
        tokenized_context = [self._tokenizer.tokenize(x) for x in context[:self._length_control]]
