...
from ontology.DataBase import DataBaseINTERFACE
...
class DataBase_SQLite(DataBaseINTERFACE):
    ...
    def _loaddb(self):
        ...
        '''
        try:
            db_connection = sqlite3.connect(self.dbfile)
            db_connection.row_factory = self._dict_factory   # for getting entities back as python dict's
    ...
    def entity_by_features(self, constraints):
        ...
        results = cursor.fetchall()        # can return directly
    ...
    def query_entity_property(self, name, slot, value):
        ...
        cursor = self._loaddb()
        cursor.execute(sql_query)
        results = cursor.fetchall()        # TODO delete -- just for debug
