...
from belieftracking.BeliefTracker import BeliefTracker
...
class FocusTracker(RuleBasedTracker):
    ...
    def _addTurn(self, turn):
        ...
        for slot in set(list(this_u.keys()) + list(hyps["goal-labels"].keys())) :
        ...
        for slot in set(list(requested_slot_stats.keys()) + list(hyps["requested-slots"].keys())):
