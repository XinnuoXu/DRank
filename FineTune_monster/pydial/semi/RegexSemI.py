...
import semi.SemIContextUtils as contextUtils
...
class RegexSemI(SemI):
    ...
    def decode(self, obs, sys_act=None, turn=None):
        ...
        for ob in obs:
            ...
            assert(isinstance(sentence,bytes) or isinstance(sentence,str))
    ...
    def init_regular_expressions(self):
        ...
        self.rBYE = "(\b|^|\ )(quit|thank|thanks|great|okay|ok|bye|goodbye|that'?s?\ (is\ )*all)(\s|$|\ |\.)"
    ...
    def _decode_bye(self, obs):
        ...
        if self._check(re.search(self.rBYE,obs, re.I)): #DEL is not None:
            is_inform = [item for item in self.semanticActs if (item.find("inform") > -1 or item.find("request") > -1)]
            if len(is_inform) == 0:
